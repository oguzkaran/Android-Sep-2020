...
public final class BitConverter {
	...
	{
		return allocate(2).putShort(value).order(ByteOrder.LITTLE_ENDIAN).getShort(0);
	}

	public static int changeEndian(int value, ByteOrder byteOrder)
	{
		return allocate(4).putInt(value).order(ByteOrder.LITTLE_ENDIAN).getInt(0);
	}

	public static long changeEndian(long value, ByteOrder byteOrder)
	{
		return allocate(8).putLong(value).order(ByteOrder.LITTLE_ENDIAN).getLong(0);
	}
}
