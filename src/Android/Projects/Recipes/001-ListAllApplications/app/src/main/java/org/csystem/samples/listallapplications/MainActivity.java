...
public class MainActivity extends AppCompatActivity {
    ...
    {
        Intent intent = m_packageManager.getLaunchIntentForPackage(packageName);
    ...
    private void loadApplications()
    {
        ...
        try {
           disposable = Observable.fromIterable(applicationInfos)
                    .filter(ai -> ai.packageName.contains("hugin"))
                    .subscribe(apps::add);
    ...
    public void onListButtonClicked(View view)
    {
        try {
            ...
            Toast.makeText(App.getInstance().getBaseContext(), ex.getMessage(), Toast.LENGTH_LONG).show();
        }
    }
}
